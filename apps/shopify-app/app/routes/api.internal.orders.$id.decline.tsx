...
import { declineOrder, addTimelineEvent } from "../services/order.server";
import { buildOrderDetailResponse } from "../services/order-detail-response.server";

/**
 * POST /api/internal/orders/:id/decline
 * Transitions AWAITING_REVIEW -> DRAFT with a decline reason.
 */
export const action = async ({ request, params }: ActionFunctionArgs) => {
  ...
  const result = await declineOrder(auth.shopId, orderId);
  if (!result.success) {
  ...
  if (!data) {
    return jsonResponse(500, { data: null, error: { code: "INTERNAL_ERROR", message: "Failed to load order after decline" } });
  }

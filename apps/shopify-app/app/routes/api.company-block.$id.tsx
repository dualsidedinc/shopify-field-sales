...
/**
 * API endpoint for the Company Admin Block extension.
 * Returns territory and sales rep information for a company.
 *
 * GET /api/company-block/:id
 * :id can be either a Shopify GID or numeric ID
 */

interface CompanyBlockData {
  companyId: string;
  companyName: string;
  territories: Array<{
    id: string;
...
export const loader = async ({ request, params }: LoaderFunctionArgs) => {
  ...
  if (!companyIdParam) {
    return cors(Response.json({ error: "Company ID required" }, { status: 400 }));
  }
  ...
  if (!shop) {
    return cors(Response.json({ error: "Shop not found" }, { status: 404 }));
  }
  ...
  if (!company) {
    return cors(Response.json({ error: "Company not found" }, { status: 404 }));
  }
  ...
  for (const location of company.locations) {
    if (location.territory && !territoriesMap.has(location.territory.id)) {
      // Find the primary rep for this territory
